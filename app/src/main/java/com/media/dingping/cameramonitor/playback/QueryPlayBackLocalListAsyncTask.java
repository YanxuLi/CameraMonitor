...
public class QueryPlayBackLocalListAsyncTask extends HikAsyncTask<String, Void, Integer> {
    ...
    private boolean onlyHasLocal = true;
    ...
    private final SimpleDateFormat yyyyMMddHHmmss = new SimpleDateFormat("yyyyMMddHHmmss", Locale.CHINA);
    ...
    protected void onPostExecute(Integer result) {
        // mWaitDlg.dismiss();
        if (!abort) {
            playBackListTaskCallback.queryTaskOver(RemoteListContant.TYPE_LOCAL, queryMode, localErrorCode, queryDetail);
...
//    	dst.setFileType(src.getFileType());
    ...
    public int searchLocalFile() throws InnerException {
        ...
        Date beginDate = null;
        Date endDate = null;
        beginDate = DateTimeUtil.beginDate(queryDate);
        endDate = DateTimeUtil.endDate(queryDate);
    ...
    private CloudPartInfoFile getCloudPartInfoFile(CloudPartInfoFile cloudPartInfoFile, Date beginDate, Date endDate) {
        ...
        cloudPartInfoFile.setStartTime(yyyyMMddHHmmss.format(beginCalender.getTime()));
        cloudPartInfoFile.setEndTime(yyyyMMddHHmmss.format(endCalender.getTime()));
